...
namespace GalaxyPanel.UsuariosPage
{
	...
	public partial class StaffsOnline : ContentPage
	{
		public StaffsOnline()
		{
			InitializeComponent();
		...
		async void GetUsers()
		{
			...
			var Api = new App();
			var jsonGalaxy = await Api.Api("/staffson/" + usuario + "/" + senha + "");
			...
			if (json.status != 1)
			{
				await DisplayAlert("Erro", "Erro ao obter staffs onlines..", "OK");
			}
			else
			{
			...
			{
				await DisplayAlert("Erro", "Erro ao obter staffs online..", "OK");
	}
}
